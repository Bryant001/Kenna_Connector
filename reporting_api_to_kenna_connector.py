#!/usr/bin/env python3
"""
reporting_api_to_kenna_connector.py
────────────────
...
API_KEY_ID     = ""    # Hard-coded crednetials in case customers want to quickly test the script with their account
API_KEY_SECRET = ""    # Leave **both** blank to use env vars or ~/.veracode/credentials file

# You can always test with App_ID 2457394, which is one of their retired App Profiles
APP_IDS     = [2457394]                                         # One or many numeric App IDs. It's the 2nd number in the app profile URL.
...
for r in rows:
    ...
    #  SCA: CVE; then SRCCLR/SRCCLR/SRCCLR-SID-* ids; else fall back to CWE
    #  Non-SCA: use CWE (since CVE is not applicable)
    srcclear_id = (
        r.get("srcclear_id")
    ...
    # Prefix exactly matches examples: "veracode <ID>"
    if r["scan_type"] == "SCA":
    ...
    if vdef_key not in vdefs:
        ...
        # Only include cve_identifiers for SCA findings
        if r["scan_type"] == "SCA" and cve_val:
